...
static void prvConfigureClocks( void )
{
	...
	// Here we set freq of main ocillator to 48MHzS
	CS_setDCOCenteredFrequency( CS_DCO_FREQUENCY_48 );
	// Here we configure each clk module to use DCO, with 1:1 ratio (divider_1)
	// high-speed subsystem clock
	CS_initClockSignal( CS_HSMCLK, CS_DCOCLK_SELECT, CS_CLOCK_DIVIDER_1 );
	// low-speed sybsystem clock (max speed 24Mhz)
	CS_initClockSignal( CS_SMCLK, CS_DCOCLK_SELECT, CS_CLOCK_DIVIDER_1 );
	// master clock for CPU
	CS_initClockSignal( CS_MCLK, CS_DCOCLK_SELECT, CS_CLOCK_DIVIDER_1 );
	// Except aclk is configured to use REFOCLK
